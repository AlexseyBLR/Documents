...
<body>
    ...
    <c:if test="${docAssets.tableHeader.visible eq '1'}">
    ...
    <table border="1">
            ...
            <c:forEach var="docAssetsListElement" items="${docAssetsList}">
                <tr>
                    <c:forEach var="attr" items="${docAssets.tableHeader.column}">
                        <c:choose>
                            <c:when test="${attr.column.size()>1}">
                                <c:forEach var="val" items="${attr.column}">
                                    <c:if test="${attr.editable eq '1'}">
                                        <td><input type="text" value="${docAssetsListElement.get(val.num)}"
                                                   style="text-align: ${docAssets.tableHeader.alignment};
                            ...
                            <c:otherwise>
                                <c:if test="${attr.editable eq '1'}">
                                    <td><input type="text" value="${docAssetsListElement.get(attr.num)}"
                                               style= "text-align: ${docAssets.tableHeader.alignment};
                                               width: ${attr.columnWidth}px"></td>
                                </c:if>
                                <c:if test="${attr.editable eq '0'}">
